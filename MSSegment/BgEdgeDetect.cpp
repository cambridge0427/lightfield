...
void BgEdgeDetect::CreateLookTable()
{
   //bgLog("Creating angle lookup table\n");
	std::cout<<"Creating angle lookup table\n"<<std::endl;
   int i;
   for (i=-180; i<=180; i++)
   {
                               ...
                               int nMin, int nmxType, int hystTypeHigh, int hystTypeLow)
{
   ...
   std::cout<<"...nonmaxima supression: "<<std::endl;
  ...
  // bgLog("...hysteresis thresholding, high: ");
   std::cout<<"...hysteresis thresholding, high: "<<std::endl;
...
void BgEdgeDetect::ComputeEdgeInfo(BgImage* cim, float* confMap, float *rank)
{
   ...
   std::cout<<"Computing confidence map...\n"<<std::endl;
   float *pGx, *pGy, *pTemp;
   BgImage tcim(x_, y_);
   if (cim->colorIm_==1)
   {
