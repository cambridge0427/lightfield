...
//	msSys.StartTimer();
//#endif
   dTime = (clock() - tStart)/ ((double)CLOCKS_PER_SEC);
   tStart = clock();
   std::cout<<"time:"<<dTime<<std::endl;
   std::cout<<"connecting regions..."<<std::endl;
...
//	msSys.StartTimer();
//#endif
	std::cout<<"Applying transitive closure..."<<std::endl;
	tStart = clock();
...
//#endif
	...
	std::cout<<"Pruning spurious regions..." <<std::endl;		
...
void msImageProcessor::SetDepth(float* const pDepth, float fDepthThresh)
{
	mpDepth = pDepth;
	mfDepthThresh = fDepthThresh;
}

// not in use
void msImageProcessor::FuseWithDepth(float* const pDepth, float fDepthThresh)
{
	mpDepth = pDepth;
	
	// initialization and checking if data needed is already prepared
	std::cout<<"fuse regions with depth..."<<std::endl;
	tStart = clock();

	//allocate memory visit table
	//visitTable = new unsigned char [L];

	bool bHasSthToMerge = true;
	int nCntMergeTimes = 0;
	do{
		bHasSthToMerge = TransitiveClosurebyDepth(fDepthThresh);
		nCntMergeTimes++;
	}while(bHasSthToMerge);
...
bool msImageProcessor::TransitiveClosurebyDepth(float fDepthThresh)
{
	BuildRAM();
	ComputeAverageDepth();
	bool bHasSthToMerge = false;
	...
	{
		//aquire first neighbor in region adjacency list pointed to by raList[i]
		neighbor	= raList[i].next;
		//traverse region adjacency list of region i, attempting to join
		//it with nbrs whose average depth is close to it.
		while(neighbor)
		{
			int nNbrLabel = neighbor->label;
			float fForDebug2 = raList[i].fAvgDepth-raList[nNbrLabel].fAvgDepth;
			//attempt to join region and neighbor...
			if(abs(raList[i].fAvgDepth-raList[nNbrLabel].fAvgDepth)<fDepthThresh)
			{
...
void msImageProcessor::ComputeAverageDepth()
{
	assert(labels!=NULL);
	assert(mpDepth!=NULL);
	double *pAccumDepth = new double[regionCount];
	memset(pAccumDepth, 0, regionCount*sizeof(double));
	for (int i=0; i<width*height; i++){
		int nLabel = labels[i];
		pAccumDepth[nLabel] += mpDepth[i];
	}
	for (int i=0; i<regionCount; i++){
		raList[i].fAvgDepth = pAccumDepth[i]/modePointCounts[i];
	}
	delete[] pAccumDepth;
	return;
}
...
void msImageProcessor::Segment(int sigmaS, float sigmaR, int minRegion, SpeedUpLevel speedUpLevel, bool bDepthAssisted)
{
...
//#endif
	...
	if (bDepthAssisted){
		...
		int nCntMergeTimes = 0;
		do{
			bHasSthToMerge = TransitiveClosurebyDepth(mfDepthThresh);
			nCntMergeTimes++;
		}while(bHasSthToMerge && nCntMergeTimes<20);

		dTime = (clock()-tStart)/ ((double)CLOCKS_PER_SEC);
		std::cout<<"time:"<<dTime<<std::endl;
		std::cout<<"mergetimes:"<<nCntMergeTimes<<std::endl;
		tStart = clock();
	}
...
//#endif
//#endif
	std::cout<<"Applying mean shift (Using Lattice)... "<<std::endl;
...
//#endif
//#endif
	std::cout<<"done.\nApplying mean shift (Using Lattice)... "<<std::endl;
...
void msImageProcessor::TransitiveClosure( void )
{
	...
	//   whose associated modes are a normalized distance of < 0.5 from one another
	...
	for(i = 0; i < regionCount; i++)
	{
		...
		//compute edge strenght threshold using global and local epsilon
...
// added by Jianqiao
// move some code from TransitiveClosure to here for reuse
void msImageProcessor::ReLabelImage()
{
	//Traverse joint sets, relabeling image.
	// (a)
	// Accumulate modes and re-compute point counts using canonical elements generated by step 2.
	...
	memset(MPC_buffer, 0, sizeof(int)*regionCount);
	memset(modes_buffer, 0, sizeof(float)*N*regionCount);
	// modified by Jianqiao to memset
	//for(int i = 0; i < regionCount; i++)
	//	MPC_buffer[i]	= 0;
	//for(int i = 0; i < N*regionCount; i++)
	//	modes_buffer[i]	= 0;
